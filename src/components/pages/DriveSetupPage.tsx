...
export default function DriveSetupPage({
  ...
  const [remote, setRemote] = useState(data.drive.config.remote ?? data.drive.rclone?.remotes?.[0] ?? "");
    ...
    <>
      ...
      <section className="panel form">
        ...
        <div className="actions">
          <button className="secondary" onClick={() => act("pick", () => window.lifeboat.pickDriveDestination())}>
            Browse
          </button>
          <button disabled={!destination || busy} onClick={() => act("test", () => window.lifeboat.testDriveDestination(destination))}>
            Test write access
          </button>
        </div>
        {data.drive.config.destination && (
          <p className="success">
            Reachable: {data.drive.config.destination} · {data.drive.config.freeBytes ? `${(data.drive.config.freeBytes / 1024 ** 3).toFixed(1)} GB` : "—"} free
          </p>
        )}
      </section>
