...
ipcMain.handle("connect", async (_e, role: AccountRole) => {
  ...
  if (role === "source" && others[0])
    validateAccountRoles(acct.email, others[0].email, [
      set.destinationEmail,
      set.fallbackEmail,
    ]);
  if (role === "destination" && others[0])
    validateAccountRoles(others[0].email, acct.email, [
      set.destinationEmail,
      set.fallbackEmail,
    ]);
  db.saveAccount(acct);
...
ipcMain.handle("disconnect", async (_e, role: AccountRole) => {
  ...
  try {
    ...
    if (auth.credentials.access_token)
      await auth.revokeToken(auth.credentials.access_token);
  } catch {}
...
ipcMain.handle("save-settings", (_e, v) => {
  ...
  if (source && dest)
    validateAccountRoles(source.email, dest.email, [
      s.destinationEmail,
      s.fallbackEmail,
    ]);
...
ipcMain.handle("export-reports", async () => {
  ...
  return exportReports(r.filePaths[0], snap, db.exportRows(), {
    ...
    manifest: db.drivePage(0, 100000),
    shared: sharedAudit(
      db.drivePage(0, 100000, true),
...
ipcMain.handle("rclone-detect", async () => {
  ...
  if (!found)
    throw new Error("rclone was not found or did not pass “rclone version”");
...
ipcMain.handle("drive-set-remote", (_e, remote: string) => {
  ...
  if (!info?.remotes?.includes(remote))
    throw new Error("Select a configured rclone remote");
...
ipcMain.handle(
  "drive-start",
  async (_e, input: { remote: string; destination: string }) => {
    const source = db.accounts().find((a) => a.role === "source");
    ...
    const confirm = await dialog.showMessageBox(win!, {
      ...
      detail: `Destination: ${destination}\n\nThis uses rclone copy and never deletes source or destination files.`,
    });
    ...
    runner.start(info.path, args, {
      ...
      exit(code, signal) {
        ...
        if (code === 0) db.markDriveCopyComplete();
        db.updateJob(
          id,
...
ipcMain.handle(
  ...
  async (_e, input: { remote: string; destination: string }) => {
    if (runner.running())
      throw new Error("Pause or finish the backup before verification");
    const info = db.setting<any>("rcloneInfo", null);
    if (!info) throw new Error("Detect rclone first");
    ...
    runner.start(info.path, args, {
      ...
      async exit(code) {
        const stats = db.driveStats(),
          ...
          rows = all
            .filter((x) => !x.is_folder && x.relative_path)
            .slice(0, 20),
          ...
          unsupported = all.filter(
            (x) => x.is_native && !x.export_extension,
          ).length,
        ...
        if (result.status !== "incomplete")
          db.markDriveVerified("rclone-check-download");
        db.updateJob(
          id,
          result.status,
          progress,
...
function gmailAccounts() {
  ...
  if (!source?.subject || !destination?.subject)
    throw new Error(
      "Reconnect both Google accounts to verify their stable identities",
    );
  ...
  if (source.subject === destination.subject)
    throw new Error(
      "Source and destination are the same underlying Google account",
    );
...
ipcMain.handle("gmail-authorize", async (_e, feature: "copy" | "settings") => {
  ...
  if (role === "destination")
    validateAccountRoles(
      db.accounts().find((a) => a.role === "source")?.email ?? "",
      acct.email,
      [set.destinationEmail, set.fallbackEmail],
    );
  db.saveAccount({
    ...acct,
    scopes: [...new Set([...(existing?.scopes ?? []), ...acct.scopes])],
...
ipcMain.handle(
  ...
  async (_e, input: { subject: string; body: string }) => {
    ...
    db.gmailLog(
      db.gmailRuns()[0]?.id ?? "settings",
      "vacation-responder-enabled",
      { source: source.email, destination: destination.email },
    );
    return result;
  },
);
