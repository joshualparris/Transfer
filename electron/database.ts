...
CREATE TABLE IF NOT EXISTS other_contacts_manifest(id TEXT PRIMARY KEY,run_id TEXT NOT NULL,source_subject TEXT NOT NULL,destination_subject TEXT NOT NULL,source_resource_name TEXT NOT NULL,semantic_fingerprint TEXT NOT NULL,available_fields_json TEXT NOT NULL,selected_for_conversion INTEGER NOT NULL,destination_resource_name TEXT,status TEXT NOT NULL,verification_status TEXT,UNIQUE(source_subject,destination_subject,source_resource_name));`);
    this.db.exec(`CREATE TABLE IF NOT EXISTS calendar_runs(id TEXT PRIMARY KEY,source_subject TEXT NOT NULL,destination_subject TEXT NOT NULL,status TEXT NOT NULL,started_at TEXT NOT NULL,completed_at TEXT,last_error TEXT);
CREATE TABLE IF NOT EXISTS calendar_map(source_subject TEXT NOT NULL,destination_subject TEXT NOT NULL,source_calendar_id TEXT NOT NULL,source_name TEXT NOT NULL,destination_calendar_id TEXT,status TEXT NOT NULL,PRIMARY KEY(source_subject,destination_subject,source_calendar_id));
CREATE TABLE IF NOT EXISTS calendar_events(id TEXT PRIMARY KEY,run_id TEXT NOT NULL,source_subject TEXT NOT NULL,destination_subject TEXT NOT NULL,source_calendar_id TEXT NOT NULL,source_event_id TEXT NOT NULL,destination_calendar_id TEXT,destination_event_id TEXT,ical_uid TEXT,summary_hash TEXT,start_json TEXT NOT NULL,end_json TEXT NOT NULL,recurrence_json TEXT NOT NULL,status TEXT NOT NULL,attempts INTEGER NOT NULL DEFAULT 0,last_error TEXT,verification_status TEXT,UNIQUE(source_subject,destination_subject,source_calendar_id,source_event_id));CREATE INDEX IF NOT EXISTS idx_calendar_events_status ON calendar_events(status);`);
  }
  ...
  private recover() {
    ...
    this.db.prepare("UPDATE calendar_events SET status='failed-retryable',last_error='Recovered after interruption' WHERE status IN ('copying','verifying')").run();
    this.db.prepare("UPDATE calendar_runs SET status='interrupted',completed_at=? WHERE status='running'").run(new Date().toISOString());
  }
  close() {
    this.db.close();
  }
