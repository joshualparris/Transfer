...
export async function authFor(role: AccountRole) {
  const saved = await tokens.get(role);
  if (!saved) throw new Error(`${role} account is not connected`);
...
export async function inventory(email: string): Promise<InventorySnapshot> {
  ...
  const snap: InventorySnapshot = {
    ...
    account: email,
    gmail: { messages: 0, threads: 0, labels: [] },
    drive: {
      usageBytes: 0,
      limitBytes: null,
      files: 0,
    ...
    contacts: { contacts: 0, groups: 0, otherContacts: null },
    calendar: {
      calendars: 0,
      owned: 0,
      shared: 0,
  ...
  try {
    ...
    snap.drive.limitBytes = about.data.storageQuota?.limit
      ? Number(about.data.storageQuota.limit)
      : null;
    ...
    do {
      const r = await api.files.list({
        ...
        pageToken,
        q: "trashed = false",
        fields: "nextPageToken,files(id,mimeType,size,ownedByMe,parents)",
        spaces: "drive",
      });
      for (const f of r.data.files ?? []) {
        ...
        if (
          ...
          snap.drive.googleNative++;
        if (f.ownedByMe) snap.drive.owned++;
        else snap.drive.shared++;
  ...
  try {
    ...
    do {
      ...
      for (const c of r.data.items ?? []) {
        if (!c.id) continue;
        ids.push(c.id);
        snap.calendar.calendars++;
        if (c.accessRole === "owner") snap.calendar.owned++;
    ...
    for (const id of ids) {
      ...
      do {
        ...
        for (const e of r.data.items ?? []) {
          snap.calendar.events++;
          if (e.recurrence?.length) snap.calendar.recurring++;
