...
export async function exportReports(
  ...
  if (phase2?.contactsManifest) await writeFile(path.join(dir,"contacts-manifest.csv"),["source_type,status,verification,photo_present",...phase2.contactsManifest.map((x:any)=>[x.source_type,x.status,x.verification_status??"",x.photo_present].map(esc).join(","))].join("\n"));
  if (phase2?.calendarManifest) await writeFile(path.join(dir,"calendar-manifest.csv"),["status,verification,recurring",...phase2.calendarManifest.map((x:any)=>[x.status,x.verification_status??"",x.recurrence_json!=="[]"].map(esc).join(","))].join("\n"));
  const h = (s: unknown) =>
      String(s).replace(
        /[&<>"']/g,
        (c) =>
  ...
  const c=phase2?.contacts?.stats??{},cal=phase2?.calendar?.stats??{},take=phase2?.preservation?.result??{},more=`<div class="card"><h2>Contacts migration</h2><p>${c.discovered??0} discovered · ${c.copied??0} paired · ${c.verified??0} verified · ${c.failed??0} failed</p></div><div class="card"><h2>Calendar migration</h2><p>${cal.discovered??0} events discovered · ${cal.copied??0} copied · ${cal.verified??0} verified · ${cal.recurring??0} recurring</p></div><div class="card"><h2>Photos + Keep Takeout</h2><p>${take.photos??0} photo/video files · ${take.keep??0} Keep files · ${take.bytes??0} bytes checksum-verified</p></div>`;
  await writeFile(path.join(dir, `${base}.html`), html+gmailHtml+more);
  return dir;
}
